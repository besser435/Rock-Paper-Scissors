def notes(): # This is so long stuff like this can be collapsed in the IDE
    '''
    RPS Game by Brandon Esser, Carter, and R-Bay. V1.6 (IFT101 version) Updated November 2021
    It looks bad, we know.

    Changes over 1.5.1 and 1.6 beta:
    WAY more organized. Everything is just in def or class (idk if its the right way)
    Found better ways of doing stuff. This eleminated some reduntant code,
    now there is only one logic and history for rigged and rigged, vs 1 for each
    Game now asks again if input was invalid
...
print(Fore.BLUE + Back.YELLOW + "Rock Paper Scissors by Brandon, Carter, and R-Bay - V1.6 IFT101 - October 2021")
